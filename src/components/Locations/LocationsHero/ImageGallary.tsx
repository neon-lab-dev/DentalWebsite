            ...
            {sliders.map((slide, idx) => (
                <div
                    ...
                    className={`cursor-pointer relative rounded-3xl bg-gray-300 duration-500 ease-in-out h-[670px] ${
                        isOpen === idx ? 'w-[300px] 2xl:w-[402px]' : 'w-[100px] 2xl:w-[118px]'
                    }`}
                    key={idx}
                >
                    {/* Image */}
    ...
    {slide.title}
                    ...
                    {isOpen === idx && (
                        <div className="absolute bottom-5 left-[50%] translate-x-[-50%] text-white">
                            <h3 className="text-white text-[30px] 2xl:text-[48px] font-bold leading-normal 2xl:leading-[66px] font-Amiri">{slide.title}</h3>
                        </div>
                    )}
                </div>
            ))}
